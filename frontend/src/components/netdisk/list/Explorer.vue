<template>
  <div class="explorer">
    <el-table
      ...
      :data="tableData"
      element-loading-text="百毒网盘正在拼命加载您的文件中"
      stripe
      border
      style="width: 100%"
    ...
    share (id) {
      ...
      .then(function (res) {
        ...
        if (res.data.errorno === 29) {
          ...
          Vue.$alert('分享地址为：http://localhost:8080/#/ShareList/' + shareId, '分享成功', {
            confirmButtonText: '确定',
            callback: action => {
              window.open('http://localhost:8080/#/ShareList/' + shareId)
            }
          })
        } else {
      ...
      .catch(function (err) {
        console.error(err)
        Vue.$message.error('分享文件失败，网络请求出错或者您填写的信息有误')
        Vue.dialogVisible = false
      })
